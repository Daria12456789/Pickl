...
export default function App() {
  ...
  useEffect(() => {
    let localUid = localStorage.getItem('noptiera_uid');
    let localAlias = localStorage.getItem('noptiera_alias');
    ...
    if (localUid && localAlias) {
      ...
      fetch('/api/auth/anonymous', { method: 'POST' })
        ...
        .then(data => {
          if (data.success) {
            localStorage.setItem('noptiera_uid', data.uid);
            localStorage.setItem('noptiera_alias', data.alias);
